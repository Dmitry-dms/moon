...
func (c *CharInfo) calcTexCoords(fontWidth, fontHeight int) {
	...
	x1 := float32((c.srcX+c.width))/float32(fontWidth)
	y0 := float32(c.srcY)/float32(fontHeight)
	y1 := float32((c.srcY-c.heigth))/float32(fontHeight)

	c.TexCoords[1] = math.Vec2{x1,y0}
	c.TexCoords[0] = math.Vec2{x1,y1}
	c.TexCoords[3] = math.Vec2{x0,y1}
	c.TexCoords[2] = math.Vec2{x0,y0}
